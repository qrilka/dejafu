...
runConcurrency sched memtype g idsrc caps ma = do
  ...
  threads <- (if rtsSupportsBoundThreads then makeBound initialThread else pure)
    threads0
  let
    ctx = Context
      { cSchedState = g
...
runThreads sched memtype ref = go Seq.empty Nothing
 where
  go sofar prior ctx
    | isTerminated
    = pure (ctx, sofar, prior)
    | isDeadlocked
    = die sofar prior Deadlock ctx
    | isSTMLocked
    = die sofar prior STMDeadlock ctx
    | otherwise
    = let
        ctx' = ctx { cSchedState = g' }
      in
        case choice of
   ...
   where
    (choice, g') = scheduleThread
      sched
      prior
      (efromList "runThreads" runnable')
      (cSchedState ctx)
    runnable' =
      [ (t, lookahead (_continuation a))
      | (t, a) <- sortOn fst $ M.assocs runnable
      ]
    ...
    isDeadlocked =
      M.null (M.filter (not . isBlocked) threads)
        && ( ((~= OnMVarFull undefined) <$> M.lookup initialThread threads)
           == Just True
           || ((~= OnMVarEmpty undefined) <$> M.lookup initialThread threads)
           == Just True
           || ((~= OnMask undefined) <$> M.lookup initialThread threads)
           == Just True
           )
    isSTMLocked =
      M.null (M.filter (not . isBlocked) threads)
        && ((~= OnTVar []) <$> M.lookup initialThread threads)
        == Just True

    unblockWaitingOn tid = fmap unblock
     where
    ...
    step chosen thread ctx' = do
      (res, actOrTrc) <-
        stepThread sched memtype chosen (_continuation thread)
          $ ctx { cSchedState = g' }
      ...
      case res of
        Right ctx'' ->
          let
            threads' =
              if (interruptible <$> M.lookup chosen (cThreads ctx''))
                /= Just False
              then
                unblockWaitingOn chosen (cThreads ctx'')
              else
                cThreads ctx''
...
stepThread sched memtype tid action ctx = case action of
  ...
  AFork n a b ->
    pure
      $ let
        ...
        in ( Right ctx
             { cThreads = goto (b newtid) tid threads'
             , cIdSource = idSource'
             }
           , Single (Fork newtid)
           )
  ...
  AIsBound c ->
    let
      isBound = isJust (_bound =<< M.lookup tid (cThreads ctx))
    in
      simple
        (goto (c isBound) tid (cThreads ctx))
  ...
  AGetNumCapabilities c -> simple (goto (c (cCaps ctx)) tid (cThreads ctx))
    $ GetNumCapabilities (cCaps ctx)

  -- set the number of capabilities
  ASetNumCapabilities i c -> pure
    ( Right ctx { cThreads = goto c tid (cThreads ctx), cCaps = i }
  ...
  ANewMVar n c -> do
    ...
    pure
      ( Right ctx
        { cThreads = goto (c mvar) tid (cThreads ctx)
        , cIdSource = idSource'
        }
  ...
  APutMVar cvar@(MVar cvid _) a c -> synchronised $ do
    ...
    simple threads'
      $ if success then PutMVar cvid woken else BlockedPutMVar cvid
  ...
  ATakeMVar cvar@(MVar cvid _) c -> synchronised $ do
    ...
    simple threads'
      $ if success then TakeMVar cvid woken else BlockedTakeMVar cvid
  ...
  ANewCRef n a c -> do
    ...
    pure
      ( Right ctx
        { cThreads = goto (c cref) tid (cThreads ctx)
        , cIdSource = idSource'
        }
  ...
  AWriteCRef cref@(CRef crid _) a c -> case memtype of
    ...
    TotalStoreOrder -> do
      ...
      pure
        ( Right ctx { cThreads = goto c tid (cThreads ctx), cWriteBuf = wb' }
        , Single (WriteCRef crid)
        )
  ...
  AAtom stm c -> synchronised $ do
    ...
    case res of
      Success _ written val ->
        let
          (threads', woken) = wake (OnTVar written) (cThreads ctx)
        in
          pure
            ( Right ctx
              { cThreads = goto (c val) tid threads'
              , cIdSource = idSource'
              }
      ...
      Retry touched ->
        let
          threads' = block (OnTVar touched) tid (cThreads ctx)
        in
          pure
            ( Right ctx { cThreads = threads', cIdSource = idSource' }
  ...
  AThrowTo t e c ->
    synchronised
      $ let
          threads' = goto c tid (cThreads ctx)
  ...
  AMasking m ma c ->
    let
      ...
      m' =
        _masking . efromJust "stepThread.AMasking" $ M.lookup tid (cThreads ctx)
    ...
    in
      simple threads' $ SetMasking False m
  ...
  ASub ma c
    | M.size (cThreads ctx) > 1 -> pure
      (Left IllegalSubconcurrency, Single Subconcurrency)
    | otherwise -> do
      (res, ctx', trace, finalDecision) <- runConcurrency
        sched
        memtype
        (cSchedState ctx)
        (cIdSource ctx)
      ...
      pure
        ( Right ctx
          ...
          , cSchedState = cSchedState ctx'
          }
        , SubC trace finalDecision
        )
 ...
 where
  ...
  stepThrow t ts act e =
    let
      some = toException e
    in
      case propagate some t ts of
        Just ts' -> simple ts' act
        Nothing
          | t == initialThread -> pure
            (Left (UncaughtException some), Single act)
          | otherwise -> do
            ts' <- kill t ts
            simple ts' act
